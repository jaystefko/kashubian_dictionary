...
const AdminScreen: NextPage = () => {
	...
	useEffect(() => {
		...
		if (!data) {
			...
			const password =
				prompt(intl.formatMessage({ id: 'fillPassword' }), '') || ''
	...
	async function deleteHandler(id: number, word: string) {
		if (id === -1) return
		if (
			confirm(`${intl.formatMessage({ id: 'deleteAssurement' })} "${word}"`)
		) {
			try {
	...
	async function openModalEditHandler(id: number) {
		...
		try {
			...
			let newWord: Partial<GatheredWord> =
				response.data?.data?.findKashubianEntry
			if (!newWord) return
			setWordId(id)
			setWord(newWord)
			setIsModalOpen(true)
