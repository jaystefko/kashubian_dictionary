		...
		if (isOpen && meaning) {
			...
			setSynonymList(
				meaning.synonyms?.map(s => ({
					id: s.synonym.id,
					definition: s.synonym.definition,
				})) || []
			)
			setAntonymList(
				meaning.antonyms?.map(a => ({
					id: a.antonym.id,
					definition: a.antonym.definition,
				})) || []
			)
				...
				<main className={styles.main}>
					<Grid container spacing={2}>
						...
						<Grid item xs={12}>
							<Paper
								elevation={3}
								sx={{ background: 'transparent', margin: '0.5rem 0' }}>
								<Grid item xs={12}>
									<h3 style={{ margin: 0, padding: '1rem 0 0 1rem' }}>
